...
inline void moments_impl_calc_mean(size_t num_examples, size_t channels,
  size_t spatial_dim, const tensor_t &in, vec_t &mean) {
...
inline void moments_impl_calc_variance(size_t num_examples, size_t channels,
  size_t spatial_dim, const tensor_t &in, const vec_t &mean, vec_t &variance) {
  ...
  vector_div(variance, std::max(float_t{1.0f},
...
inline void moments(const tensor_t &in,
  size_t spatial_dim, size_t channels, vec_t &mean) {
...
inline void moments(const tensor_t &in, size_t spatial_dim,
  ...
  const size_t num_examples = in.size();
  assert(in[0].size() == spatial_dim * channels);
  moments(in, spatial_dim, channels, mean); // calc mean
  variance.resize(channels);
  vectorize::fill(&variance[0], variance.size(), float_t{0.0});
  
  detail::moments_impl_calc_variance(num_examples,
    channels, spatial_dim, in, mean, variance);
}
