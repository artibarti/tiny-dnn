...
class serialization_helper {
 ...
 private:
	...
	static void save_layer_impl(OutputArchive &oa, const layer *layer) {
		oa(cereal::make_nvp(serialization_helper<OutputArchive>::get_instance()
			.type_name(typeid(T)), *dynamic_cast<const T *>(layer)));
	}
...
void layer::save_layer(OutputArchive &oa, const layer &l) {
	const std::string &name = serialization_helper<OutputArchive>
		::get_instance().type_name(typeid(l));
	serialization_helper<OutputArchive>::get_instance().save(name, oa, &l);
}

template <class Archive>
void layer::serialize_prolog(Archive &ar) {
	std::string name = serialization_helper<Archive>
		::get_instance().type_name(typeid(*this));
